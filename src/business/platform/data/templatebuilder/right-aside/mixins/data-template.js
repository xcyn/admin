...
export default {
  ...
  methods: {
      ...
      getSelectorDataByKey({
        ...
        if (this.$utils.isNotEmpty(data.resultColumns)) {
          ...
          if ((key === 'value_source' || key === 'linkdata') && this.$utils.isNotEmpty(resultColumns)) {
            const linkConfig = this.formData && this.formData.field_options ? this.formData.field_options.link_config : {}
     ...
     */
    setFormFields() {
      this.formFields = FormFieldUtil.getFormField(this.fields, this.isSub, this.code, this.formData ? this.formData.id : '')
      this.formSubFields = this.isSub ? FormFieldUtil.getFormSubField(this.fields, this.code, this.formData ? this.formData.id : '') : this.formFields
    },
    setFormLabelFields() {
      this.formLabelFields = FormFieldUtil.getFormField(this.fields, this.isSub, this.code, this.formData ? this.formData.id : '', 'label')
    },
    ...
    settingDataTemplateConfig(key) {
      ...
      if (this.$utils.isEmpty(this.formData.field_options[key])) {
    ...
    setDataTemplateConfig(data) {
      this.formData.field_options.link_config = data
    },
    setDataTemplateCondition(data) {
      this.formData.field_options.link_condition = data
    },
    setDataTemplateLinkData(data) {
      this.formData.field_options.link_linkage = data
    },
    setDataTemplateLinkAttr(data) {
      this.formData.field_options.link_attr = data
    },
    setDataTemplateDefaultValue() {
      this.formData.field_options.link_config = null
      this.formData.field_options.link_condition = null
      this.formData.field_options.link_linkage = null
      this.formData.field_options.link_attr = null
    }
  }
}
