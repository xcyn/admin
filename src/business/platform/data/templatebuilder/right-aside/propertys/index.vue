  ...
  <div class="template-property">
    ...
    >
      ...
      <div v-if="(dataTemplate.showType === 'compose'&&dataTemplate.composeType === 'treeForm')||dataTemplate.showType!== 'compose'" class="panel panel-default">
        ...
        <div class="panel-body">
          <template v-if="(dataTemplate.showType === 'compose'&&dataTemplate.composeType === 'treeForm')||dataTemplate.showType!== 'compose'">
            ...
            <el-form-item label="详情表单" prop="form">
              <form-def-selector
                v-model="detailFormKey"
                :data-template-key="dataTemplate.key"
                :rights-type="rightsType"
                type="dataTemplate"
                :is-save="isSave"
                :toolbar="detailFormToolbar"
                value-key="key"
                form-type="detail"
              />
            </el-form-item>
...
export default {
  ...
  watch: {
    ...
    dataTemplate: {
      handler(val, oldVal) {
        if (val !== oldVal) {
          ...
          if (this.$utils.isNotEmpty(attrs)) {
            this.$nextTick(() => {
              if (this.$utils.isNotEmpty(attrs.form_key)) {
                const curFormKey = attrs.form_key || ''
                if (this.formKey !== curFormKey) {
                  this.formKey = curFormKey
                }
              }
              if (this.$utils.isNotEmpty(attrs.detail_form_key)) {
                const curFormKey = attrs.detail_form_key || ''
                if (this.detailFormKey !== curFormKey) {
                  this.detailFormKey = curFormKey
                }
              }
            })
          }
    ...
    detailFormKey: {
      handler(val, oldVal) {
        if (val !== oldVal) {
          this.handleAttrs('detail_form_key', val)
        }
      }
    }
  },
  beforeDestroy() {
    this.options = null
    this.templateTypeOptions = null
    this.showTypeOptions = null
    this.composeTypeOptions = null
  ...
  methods: {
    ...
    changeDataset(data) {
      this.$emit('is-same-dataset-key', data)
