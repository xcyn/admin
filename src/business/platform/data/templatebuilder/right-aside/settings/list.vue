<template>
  <div>
    ...
    >
      <template slot="edit" slot-scope="scope">
        <filter-condition :data="scope.data" :display="datasetType==='thirdparty'" :fields="fields" />
      </template>
    </column>
    <!--排序字段-->
    <column
    ...
    >
      <template slot="edit" slot-scope="scope">
        <editor-button
          ...
          :template="template"
          :form-key="formKey"
          :button-codes="scope.buttonCodes"
          type="function"
...
import AttrList from '../components/attr-list/index.vue'
...
export default {
  ...
  beforeDestroy() {
    this.queryDefaultValue = null
    this.displayDefaultValue = null
    this.resultDefaultValue = null
    this.filterConditionDefaultValue = null
    this.sortDefaultValue = null
    this.fucntionButtonDefaultValue = null
    this.functionListButtonTypes = null
    this.editButtonTypes = null
  },
  methods: {
    ...
    handleButtons(key, data, position) {
      const curTemplate = JSON.parse(JSON.stringify(this.template))
      if (!curTemplate.buttons) {
        curTemplate.buttons = {}
      }
      curTemplate.buttons[key] = data
      if (this.$utils.isNotEmpty(position)) {
        curTemplate.attrs.buttons_position = position
      }
      this.$emit('update', curTemplate)
    },
    handleRights(key) {
