...
export default {
  ...
  methods: {
    ...
    handleClickTree() {
      this.$emit('cloum-click', this.dataTemplate.composeType === 'treeList' ? 'composeTree' : 'composeList')
    },
    handleClickList() {
      this.$emit('cloum-click', this.dataTemplate.composeType === 'treeList' ? 'composeList' : 'composeTree')
    }
  }
}
</script>
