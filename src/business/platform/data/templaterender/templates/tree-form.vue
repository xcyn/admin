<template>
  ...
  >
      ...
      >
        ...
        >
          <el-row>
            ...
            <el-col :span="6">
              <el-button type="primary" class="ibps-tree-search-buttom ibps-icon ibps-icon-search" @click="filter">搜索</el-button>
            </el-col>
          </el-row>
          <!-- size="small"  -->
        </div>
    ...
    <ibps-container
      :margin-left="isExpand?'301px':'30px'"
      class="page"
    >
...
export default {
  ...
  methods: {
    ...
    initUI() {
      ...
      if (!this.initialization) {
        ...
        if (this.dataTemplate.attrs && this.dataTemplate.attrs.script) {
          this.eventsUtil = eventsUtil._initEvents(this.dataTemplate.attrs.script, 'tree_form_' + this.dataTemplate.key, true)
        }
        setTimeout(() => {
          this.loadScript()
        }, 10)
    ...
    handleContextmenuClick(item) {
      this.handleAction(item.key, 'contextmenu', this.contextmenuData[this.pkKey], this.contextmenuData)
    },
     ...
     * 添加、编辑表单
     */
    handleEdit(selection, command, position, data) {
      if (this.$utils.isEmpty(this.formKey)) {
        ActionUtils.warning('请绑定表单')
        return
      }
      this.defaultFormData = command === 'add' ? this.getDefaultFormData(selection) : null
      this.readonly = command === 'detail'
      const editToolbars = []
      this.editButtons.forEach(rf => {
        const btn = this.buildButton(rf)
        // 编辑页顶部按钮
        if (hasButton(rf.button_type, command === 'detail' ? 'detail' : 'edit', rf.position)) {
          // 明细页是否有按钮
          if ((!this.readonly) || (
      ...
      this.pkValue = command === 'add' ? null : (selection || '')
      this.dialogFormVisible = true
      this.afterScript(command, position, this.pkValue, data, () => {
        this.dialogFormVisible = true
      })
    },
    /**
    ...
    handleExpandCollapse() {
      ...
      setTimeout(() => {
        this.$refs.formrender.resize()
      }, 100)
    },
    /**
     * 删除表单
     */
    handleRemove(ids, command, position, selection, data) {
      if (this.$utils.isEmpty(this.formKey)) {
        ActionUtils.warning('请绑定表单')
        return
      }
      removeFormData({
        ...
        this.afterScript(command, position, selection, data, () => {
