...
export function buildDynamicParams(conditions) {
  ...
  const filter = filterConditions ? filterConditions.filter : null
  if (filter) {
...
export function buildAllDynamicParams(conditions) {
  const dynamicParams = {}
  if (Utils.isEmpty(conditions)) {
    return dynamicParams
  }
  for (let i = 0; i < conditions.length; i++) {
    const filterConditions = Utils.isNotEmpty(conditions[i]) ? conditions[i] : {}
...
export function _createDynamicParams(params, filter) {
  ...
  for (let _i = 0, _len = rules.length; _i < _len; _i++) {
    const rule = rules[_i]
    if (rule.rules) {
      _createDynamicParams(params, rule)
    } else {
...
export function buildLinkLabelTitle(titleType = 'first', dataTitle, lableKey) {
  if (titleType === 'first') {
    ...
    return function(data) {
      const d = dataTitle.split(new RegExp(FormOptions.t.COMMON_REG.VALIDATOR, 'g'))
      const rtn = []
      d.forEach(n => {
        let a = n
        if (/^\$(_widget_)/.test(n)) { // 对字段进行处理
          const f = n.replace('$_widget_', '').split('#')
          a = Utils.isNotEmpty(data[f[0]]) ? data[f[0]] : ''
        }
        rtn.push(a)
      })
