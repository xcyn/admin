<template>
  ...
  >
    ...
    <div slot="footer" class="el-dialog--center">
     <!-- <el-button
        v-if="!readonly"
        v-permission="accessControl?identity+'_bpmTaskChange':null"
        type="primary"
...
import * as defectHttp from '@/api/cpApi/defectList'
import * as transferHttp from '@/api/cpApi/shiftduty/tasktransfer'
...
export default {
  ...
  beforeDestroy() {
    ...
    this.changeTypeStatusOptions = null
  ...
  async saveData() {
      ...
      if (this.dfId){
       let res = await defectHttp.getApproveOperateInfo({dfId:this.dfId,changeType:'zb',applyer:data.bpmTaskChangeAssignPoList[0].executor})
        if (res.data.changeList.length > 0){
          ActionUtils.warning(this.shfitDes)
          loading.close()
          return
        }
      }
      if (this.sdId){
       let res = await transferHttp.getTaskTransfer({taskId:this.sdId,transferId:data.bpmTaskChangeAssignPoList[0].executor})
        if (res.data > 0){
          ActionUtils.warning(this.shfitDes)
          loading.close()
          return
