...
export default {
  ...
  beforeDestroy() {
    this.selectNodeUserAll = null
    this.selectNodeUser = null
    this.isIndeterminate = null
    this.checkAll = null
    this.nodeSupportSelectPath = null
    this.outgoingNodes = null
    this.outgoingNodesUsersMap = null
  },
  methods: {
    getFormData() {
    ...
    handleSave() {
      ...
      const destination = this.selectPathAll ? this.selectPathLine : ''

      let nodeUsers = []
      if (this.selectUserAll && this.$utils.isNotEmpty(this.outgoingNodes)) {
    ...
    getNodeUsers() {
      ...
      for (let i = 0; i < this.outgoingNodes.length; i++) {
        ...
        if (this.$utils.isEmpty(executors)) {
          this.$message({
            message: '请选择指定执行人',
            type: 'warning'
          })
