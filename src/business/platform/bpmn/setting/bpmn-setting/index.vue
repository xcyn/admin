...
export default {
  name: 'BpmDefinitionSettingSetting',
  components: {
    Global,
    StartEvent,
    EndEvent,
    ...
    cleanFormData() {
      ...
      if (this.data.global) {
        if (this.data.global.instForm && this.data.global.instForm.type === 'inner') {
          this.$set(this.data.global.instForm, 'formValue', '')
          this.$set(this.data.global.instForm, 'mobileValue', '')
          this.$set(this.data.global.instForm, 'editFormRights', '')
        }
        if (this.data.global.form && this.data.global.form.type === 'inner') {
          this.$set(this.data.global.form, 'formValue', '')
          this.$set(this.data.global.form, 'mobileValue', '')
          this.$set(this.data.global.form, 'editFormRights', '')
        }
        if (this.data.global.bo.saveMode === 'none') {
          this.$set(this.data.global.form, 'type', 'none')
          this.$set(this.data.global.instForm, 'type', 'none')
        }
      }
      ...
      for (let i = 0; i < this.data.nodes.length; i++) {
        ...
        if ((nodeData.node_type === 'userTask' || nodeData.node_type === 'signTask') && nodeData.form) {
          if (this.data.nodes[i].form && this.data.nodes[i].form.type === 'inner') {
            this.$set(this.data.nodes[i].form, 'formValue', '')
            this.$set(this.data.nodes[i].form, 'mobileValue', '')
            this.$set(this.data.nodes[i].form, 'editFormRights', '')
          }
          if (this.data.global.bo.saveMode === 'none') {
            this.$set(this.data.nodes[i].form, 'type', 'none')
          }
        } else if (nodeData.node_type === 'callActivity') {
          // 外部子流程 中的表单设置为空
          this.$set(this.data.nodes[i].callActivity, 'setting', this.cleanCallActivityFormData(this.data.nodes[i].callActivity.setting))
        }
      }
    },
    // 清空子流程的表单
