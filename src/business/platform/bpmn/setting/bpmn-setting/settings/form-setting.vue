<template>
  <div class="panel-body">
    <el-form ref="formSet" v-model="form" label-width="100px" @submit.native.prevent>
      ...
      <el-form-item v-if="form.type!='none'" label="表单：" prop="formValue">
        <form-def-selector
      ...
      <template v-if="(formType==='flow' || formType==='node') && form.type!='none'">
        <el-form-item label="前置处理器：" prop="prevHandler">
          <el-input v-model="formAttributes.prevHandler" placeholder="前置处理器" type="textarea" rows="2" autosize />
        </el-form-item>
        <el-form-item label="后置处理器：" prop="postHandler">
          <el-input v-model="formAttributes.postHandler" placeholder="后置处理器" type="textarea" rows="2" autosize />
        </el-form-item>
      </template>
    </el-form>
    <!-- 表单意见-->
...
export default {
  ...
  beforeDestroy() {
    this.formTypeOptions = null
    this.formOpinionsData = null
