<template>
  ...
  >
          ...
          <div>
            ...
            <el-col :span="11">
              <el-form-item v-if="isSupervise === 'Y'" label="开始督办次数:" prop="triggerRemindCount">
                <el-select v-model="triggerRemindCount" placeholder="请选择" @change="validateCount">
                  <el-option
                    v-for="item in timesOptions"
                    :key="item.value"
                    :label="item.label"
            ...
            <el-col :span="9" :offset="1">
              ...
              <el-button
                v-if="isSupervise === 'Y'"
                type="primary"
                @click="handleClearSuperviseData
                "
              >清除设置</el-button>
            </el-col>
          </div>
    ...
    <supervise-dialog
      ...
      :data="form.supervise"
      :node-id="form.nodeId"
      :rel-node-id="form.relNodeId"
      :rel-node-event="form.relNodeEvent"
      style="width:'100%'"
      class="form-content"
...
export default {
  ...
  methods: {
    ...
    validateCount() {
      if (this.triggerRemindCount > this.form.sendTimes) {
        this.$message.closeAll()
        this.$message({
          message: '开始督办次数不能大于发送次数',
          type: 'warning'
        })
      }
    }
