<template>
  <div class="bpm-user-plugins-hr-script">
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            <bpmn-form-var size="mini" style="display: inline-block;margin-left:10px;" @input="handleFormVar" />
          </td>
        </tr>
        <tr>
    ...
    <common-script
      ...
      :style="{width:'100%'}"
      class="form-content"
      @close="() => commonScriptVisible = false"
    ...
    <condition-script-select
      ...
      :style="{width:'100%'}"
      class="form-content"
      @close="() => conditionScriptSelectVisible = false"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleActionEvent(key, data) {
      ...
      if (key === 'confirm') {
        // todo  data数据如何适应到formData
        this.commonScriptVisible = false
      }
    }
  }
}
