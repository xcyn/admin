    ...
    <el-row>
      ...
      <el-col v-if="!isMaster" :span="24">
        <el-form-item label="关系类型：" prop="relation">
          <el-select v-model="form.relation" :disabled="(readonly||(!form.edit&&pass)||(isCreateTable==='Y'&&!form.edit))||!form.edit" style="width:100%;">
            <el-option
  ...
  watch: {
    ...
    formData: {
      handler: function(val) {
        ...
        const rootIndex = this.changeOptions.findIndex(this.changeOptions.find(e => e.code === this.form.pk))
    ...
    changeOptions: { // 初始化下拉数据。
      handler: function(val, oldVal) {
        ...
        this.form.fk = this.form.fk === null || this.form.fk === '' ? fkIndex > -1 ? val[fkIndex].code : '' : this.form.fk
        const hanFk = val.findIndex(item => item.code === this.form.fk)
        if (hanFk === -1) { this.form.fk = '' }
      },
      deep: true
    }
  },
