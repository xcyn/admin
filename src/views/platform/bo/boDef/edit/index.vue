<template>
  ...
  >
    <div slot="title" style="padding-left: 20px;" class="el-dialog--left">
      ...
      <el-button v-if="formId&&!readonly&&entrance!=='moreVersions'" v-permission="BODEF+'_deploy'" icon="ibps-icon-release" type="primary" @click="handleSave('deploy')">发布新版本</el-button>
    </div>
...
export default {
  ...
  data() {
    return {
      dialogVisible: false,
      BODEF: Identity.IBPS.FORM.BODEF,
      dialogLoading: false,
      isMaster: true,
      isAdd: false,
      sameCode: false,
      ...
      treeContextmenus: [
        ...
        { icon: 'add',
          ...
          rights: (menu, data, isRoot) => {
            if (this.$utils.isNotEmpty(data) && data.relation === 'one2one') return false
  ...
  methods: {
    ...
    // 设置初始值-关系类型-禁用
    setDisabled(dataParams) {
      const filter = function(data) {
        data.find(item => {
          if (item.subBoDefs && item.subBoDefs.length > 0) {
            item.subBoDefs.forEach(s => {
              s.edit = false
              if (s.subBoDefs && s.subBoDefs.length > 0) {
                filter(s.subBoDefs)
              } else {
                return
              }
    ...
    getFormData() {
      ...
      if (this.$utils.isEmpty(this.formId)) {
        ...
        manage({
          ...
          this.treeData[0][this.childrenKey] = this.setDisabled([data])
