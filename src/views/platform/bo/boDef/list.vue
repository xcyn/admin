...
export default {
  ...
  data() {
    return {
      identity: Identity.IBPS.FORM.BODEF,
      moreVersionsIdentity: Identity.IBPS.FORM.BODEFDIALOGMOREVERSIONS,
      width: 200,
      BODEF_TREE: Identity.IBPS.FORM.BODEF_TREE,
  ...
  methods: {
    ...
    loadData() {
      this.loading = true
      queryPageList(this.getFormatParams(this.typeId)).then(response => {
    ...
    handleExport(ids) {
      this.$confirm('是否进一步配置导出数据？否则直接导出当前业务对象。', '业务对象导出', {
        confirmButtonText: '是',
        cancelButtonText: '否',
    ...
    handleRemove(id) {
      remove({
        ...
        if (this.$utils.isNotEmpty(response.variables)) {
          this.$message.closeAll()
          this.$confirm(
            '<div style="overflow-y:auto;max-height:600px;">' + response.cause + '<br><b>继续删除就只删除没有关联父对象、表单、流程、表配置的业务对象,是否继续删除？</b></div>',
            '提示',
            {
            ...
            if (this.$utils.isNotEmpty(response.variables.canDeleteBoIds)) {
              remove({
                boDefIds: response.variables.canDeleteBoIds,
                rmType: 'all',
                clean: true
              }).then(response => {
                this.loadData()
