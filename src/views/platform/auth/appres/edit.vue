<template>
  ...
  >
    <template slot="header">
      <el-button v-permission="APPRES+'_save'" type="primary" icon="ibps-icon-save" @click="handleSave()">保存</el-button>
    </template>
    <el-form ref="resourcesForm" :model="resources" :rules="rules" :label-width="formLabelWidth" @submit.native.prevent>
      ...
      <el-form-item label="数量接口:" prop="countInterface">
        <el-input v-model="resources.countInterface" autocomplete="off" />
      </el-form-item>
      <el-form-item label="请求方式:" prop="requestMode">
        <el-select v-model="resources.requestMode">
          <el-option
...
export default {
  ...
  data() {
    return {
      loading: false,
      APPRES: Identity.IBPS.SYSTEM.APPRES,
