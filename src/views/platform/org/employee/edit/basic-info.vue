      ...
      <el-form ref="employeeForm" :rules="rules" :model="formData" label-width="130px" @submit.native.prevent>
        ...
        <el-form-item label="是否管理员" prop="isSuper">
          <el-switch
            ...
            :disabled="(formData.id===''||$store.getters.userId!==formData.id)&&isSuper?false:true"
            active-value="Y"
            inactive-value="N"
          />
        ...
        <el-form-item prop="wcAccount">
          <span slot="label">
            企业微信账户
            <el-tooltip
              class="item"
              effect="light"
              content="需填写企业微信账户，而不是个人微信账户！"
              placement="bottom"
            >
...
export default {
  ...
  data() {
    ...
    const validateAccount = (rule, value, callback) => {
      const re = /^[a-zA-Z][0-9a-zA-Z]{1,64}$/
      const testNum = /^[0-9]{1,64}$/
      if (!re.test(value) && !testNum.test(value)) {
    ...
    return {
      ...
      rules: {
        account: [
          ...
          { type: 'string', min: 3, max: 64, message: '帐号长度至少3位,至多64位', trigger: 'blur' },
