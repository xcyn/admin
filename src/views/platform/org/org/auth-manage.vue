...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      orgTreeContextmenus: [
        { icon: 'add', label: '添加', key: 'add', rights: this.checkRights },
        { icon: 'edit', label: '编辑', key: 'edit', rights: this.checkRights },
        { icon: 'remove', label: '删除', key: 'remove', rights: this.checkRights },
        { type: 'divided', divided: true, rights: this.rights },
        { icon: 'arrows-v', label: '移动节点', key: 'moveNode', rights: this.rights },
        { type: 'divided', divided: true, rights: this.rights },
        { icon: 'cog', label: '设置分级管理员', key: 'authEdit', rights: this.rights },
        { icon: 'add', label: '添加岗位', key: 'addPosition', rights: this.rights }
      ],
      readonly: false,
      orgTreeData: [],
      partyOrgAuths: [],
      AUTH_ORG_TREE: Identity.IBPS.ORG.AUTH_ORG_TREE,
      Identitys: {
        CHARGE: Identity.IBPS.ORG.AUTH_ORG_CHARGE,
        POSITION: Identity.IBPS.ORG.AUTH_ORG_POSITION,
        EMPLOYEE: Identity.IBPS.ORG.AUTH_ORG_EMPLOYEE,
        EXTATTR: Identity.IBPS.ORG.AUTH_ORG_EXTATTR,
        ROLE: Identity.IBPS.ORG.AUTH_ORG_ROLE,
        AUTH: Identity.IBPS.ORG.AUTH_ORG_AUTH
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    checkRights(menu, data, isRoot) {
      ...
      if (this.$utils.isNotEmpty(prem) && prem.indexOf(menu.key) > -1) {
        return true
      } else {
        return false
      }
