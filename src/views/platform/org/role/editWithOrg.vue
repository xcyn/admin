<template>
  ...
  >
    ...
    >
      ...
      <el-form-item label="所属组织:" prop="subSystemId">
        <el-select v-if="!readonly" v-model="role.companyId" placeholder="请选择" style="width:100%;">
          <el-option
        ...
        <span v-else>{{ role.companyId|optionsFilter(companyList,'name','id','所有系统') }} </span>
      </el-form-item>
...
import { saveRoleOrg,getByRoleId } from '@/api/platform/org/cpisRoleOrg'

import { findAllSubsystem ,getCompanyByCurrentUser } from '@/api/platform/auth/subsystem'
...
export default {
  ...
  methods: {
    ...
    loadCompanyData(){
      getCompanyByCurrentUser().then(response => {
        this.companyList = response.data
        if (this.$utils.isEmpty(this.formId) &&
          this.$utils.isNotEmpty(this.companyList)) {
          this.roleOrg.companyId = this.companyList[0].id
        }
      })
    },
    // 保存数据
    ...
    saveData() {
      save(this.role).then(response => {
        this.roleOrg.superType = response.variables.id
        saveRoleOrg(this.roleOrg).then(responseOrg => {
          this.$emit('callback', this)
          ActionUtils.saveSuccessMessage(responseOrg.message, (rtn) => {
            if (this.$utils.isEmpty(this.formId)) {
    ...
    getFormData() {
      ...
      get({
        ...
        getByRoleId({
          superType: this.formId
        }).then(responseOrg => {
          this.roleOrg = responseOrg.data
          this.role.companyId = responseOrg.variables.companyId
        }).catch(() => {
