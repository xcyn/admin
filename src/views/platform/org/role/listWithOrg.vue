...
<script>
import { queryPageListWithOrg, remove } from '@/api/platform/org/role'
...
export default {
  ...
  data() {
    return {
      identity: Identity.IBPS.ORG.ROLE,
      identitys: {
      ...
      listConfig: {
        ...
        searchForm: {
          forms: [
            ...
            {
              prop: 'Q^org.company_id^SL',
              label: '公司',
              fieldType: 'select',
              valueKey: 'id',
              labelKey: 'name',
              resultField: 'data',
              selectFetch: getCompanyByCurrentUser
            }
          ]
        },
        // 表格字段配置
  ...
  methods: {
    ...
    loadData() {
      this.loading = true
      queryPageListWithOrg(this.getSearcFormData()).then(response => {
    ...
    handleAction(command, position, selection, data) {
      switch (command) {
        ...
        case 'authApiGrant':// 接口授权申请
          ...
          break
        case 'authDataRole':// 资源分配
          this.handleAuthDataRole(selection, data.subSystemId, '')
          break
        default:
          break
      }
    ...
    handleAuthDataRole(id, systemId, type) {
      this.assignResourceType = type
      this.editId = id
      this.systemId = systemId
      this.openTab(id, systemId)
    },
    openTab(id, systemId) {
      this.$router.push({
        name: 'authData-allPermission',
        query: {
          roleId: id,
          systemId: systemId
        }
