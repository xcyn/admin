  ...
  >
    ...
    >
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="展示效果:" prop="showEffect">
            <el-select v-if="!readonly" v-model="desktopColumn.showEffect" placeholder="请选择" style="width:100%;" @change="changeShowEffect">
              <el-option
      ...
      <el-row>
        ...
        <el-col v-if="desktopColumn.showEffect===0" :span="6">
          <el-form-item label="是否分页:" prop="needPage">
            <el-radio-group v-if="!readonly" v-model="desktopColumn.needPage" @change="changeNeedPage">
              <el-radio
        ...
        <el-col v-if="desktopColumn.needPage" :span="6">
          <el-form-item label="每页条数:" prop="pageLimit">
            <el-input-number v-if="!readonly" v-model="desktopColumn.pageLimit" controls-position="right" :min="1" />
            <span v-else>{{ desktopColumn.pageLimit }}</span>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        ...
        <el-col :span="12" style="height:51px;">
          <el-form-item label="支持刷新:" prop="supportRefesh">
            ...
            <template v-if="desktopColumn.supportRefesh">
              <el-input v-if="!readonly" v-model="desktopColumn.refeshTime" placeholder="请输入相隔时长" style="width:220px;margin-left:20px;">
                <template slot="append">
                  <el-select v-model="desktopColumn.timeUnit" placeholder=" " style="width:90px;">
                    <el-option
              ...
              <span v-else> {{ desktopColumn.refeshTime }} {{ desktopColumn.timeUnit|optionsFilter(timeOptions,'label') }}</span>
            </template>
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
import { typeOptions, statusOptions, dataModeOptions, showEffectOptions, booleanOptions, timeOptions } from './constants'
...
export default {
  ...
  data() {
    const validateNum = (rule, value, callback) => {
      const re = /^[0-9]{1,10}$/
      if (value && !re.test(this.desktopColumn.refeshTime)) {
        if (this.$utils.isEmpty(this.desktopColumn.refeshTime)) {
          callback(new Error('请输入时长'))
        } else if (this.desktopColumn.refeshTime.length > 10) {
          callback(new Error('时长整数位长度不能超过10'))
        } else {
          callback(new Error('时长请输入整数！'))
        }
      } else {
        callback()
      }
    ...
    const validateHeigh = (rule, value, callback) => {
      const re = /\D/
      if (re.test(value)) {
    ...
    return {
      ...
      rules: {
        ...
        colHeight: [{ required: true, message: this.$t('validate.required') },
          { validator: validateHeigh, trigger: 'blur' }]
      },
      toolbars: [
  ...
  watch: {
    ...
    'desktopColumn.templateHtml': {
      handler: function(val, oldVal) {
        this.formValidate()
      },
      immediate: true
    },
    'desktopColumn.supportRefesh': {
      handler: function(val, oldVal) {
        if (val) {
          this.formValidate()
        }
  ...
  methods: {
    ...
    changeNeedPage(val) {
      if (!val) {
        this.desktopColumn.pageLimit = 10
      }
    },
    changeSupportRefesh(val) {
      if (val) {
        this.desktopColumn.refeshTime = 1
        this.desktopColumn.timeUnit = 'minute'
      }
    },
    changeShowEffect(val) {
      if (val !== 0) {
        this.desktopColumn.needPage = false
      }
    },
    // 保存数据
    handleSave() {
