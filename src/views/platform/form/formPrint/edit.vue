...
export default {
  ...
  methods: {
        ...
        ActionUtils.saveSuccessMessage(response.message, (rtn) => {
          ...
          if (rtn) {
            ...
            if (this.$utils.isEmpty(this.formId)) {
              this.formId = response.variables.id
            }
          }
        })
      }).catch(() => {
    ...
    onDrop(e) {
      const data = e.dataTransfer.getData('fieldData')
      if (data) {
    ...
    getTableValue() {
      ...
      const countRows = printTable.countRows()
      const countCols = printTable.countCols()
      const merge = {}
      const g = {}
      ...
      for (let i = 0; i < mergeCells.length; i++) {
        ...
        if (!(mergeCell.row > countRows || mergeCell.col > countCols)) {
          const h = this._getCellTag(mergeCell.row, mergeCell.col)
          const j = this._getCellTag(mergeCell.row + mergeCell.rowspan - 1, mergeCell.col + mergeCell.colspan - 1)
          merge[h] = j
      ...
      for (let i = 0; i < cellsMeta.length; i++) {
        ...
        if (cellMeta && this.$utils.isNumber(cellMeta.row) && this.$utils.isNumber(cellMeta.col)) {
          const tag = this._getCellTag(cellMeta.row, cellMeta.col)
          if (g[tag]) {
            continue
          }
          let content = this.dataMap[tag]
          const styleKey = this.printUtils.getStyleNameByCell(cellMeta)
          let hasField = false
          let hasContent = false
          const hasBorder = this.printUtils.hasBorder(styleKey)
          ...
          if (hasContent || hasBorder) {
            ...
            let row = cellMeta.row
            let col = cellMeta.col
            if (merge[tag]) {
              const r = this._splitCellTag(merge[tag])
              row = r[0]
              col = r[1]
            }
            if (row > maxRow) {
    ...
    setTableValue(value) {
      ...
      for (const key in value.cells) {
        ...
        const tag = _this._splitCellTag(key)
      ...
      let minRows = options.minRows
      const f = []
      for (const key in value.merge) {
        const val = value.merge[key]
        const d = _this._splitCellTag(key)
        const g = _this._splitCellTag(val)
        const h = g[0] - d[0] + 1
        const i = g[1] - d[1] + 1
        if (h > 1 || i > 1) {
          f.push({
    ...
    _setCellData: function(row, col, value) {
      ...
      if (!this.tableData) {
        this.tableData = []
        const d = this._splitCellTag(this.tableValue.range.e)
        for (let i = 0; i <= d[0]; i++) {
          this.tableData[i] = []
    ...
    _splitCellTag(tag) {
      ...
      const row = parseInt(tags[0])
      const col = parseInt(tags[1])
    ...
    _addTableHook() {
      ...
      printTable.addHook('afterColumnResize', function(col, newSize) {
        ...
        // const e = new Handsontable.plugins.UndoRedo.ColWidthAction({
        //   col: col,
      ...
      printTable.addHook('afterRowResize', function(row, newSize) {
        ...
        // const e = new Handsontable.plugins.UndoRedo.RowHeightAction({
        //   row: row,
      ...
      printTable.addHook('beforeChange', (changes, source) => {
        changes.forEach(([row, column, oldValue, newValue]) => {
          const tag = _this._getCellTag(row, column)
          let val = void 0
      ...
      // 右键菜单
      printTable.addHook('afterContextMenuShow', (context) => {
        context.menu.container.style.zIndex = _this.zIndex
      })
    },
    ...
    _setMenuState(row, col, row2, col2) {
      const cell = this.getPrintTable().getCellMeta(row, col)
      if (this.printUtils) {
    ...
    setSize(options) {
      const printTable = this.getPrintTable()
      const selected = printTable.getSelected()
      if (!selected) { return }
    ...
    insertOrDelete(options) {
      const selected = this.getPrintTable().getSelected()
      if (selected) {
        const countRows = this.getPrintTable().countRows()
        const countCols = this.getPrintTable().countCols()
        const selRow = Math.abs(selected[0] - selected[2])
        const selCol = Math.abs(selected[1] - selected[3])
        const isRow = countRows - 1 <= selRow
        const isCol = countCols - 1 <= selCol
    ...
    removeColumn() {
      ...
      if (range) {
        const end = range.end.col - range.start.col + 1
    ...
    removeRow() {
      const range = this._getSelection()
      if (range) {
        const end = range.end.row - range.start.row + 1
    ...
    _getCellText(value) {
      ...
      if (this.$utils.isArray(value)) {
        ...
        const image = this.imageMap[value.image] || value.image
        if (image) {
    ...
    clearCellData() {
      const selection = this.getPrintTable().selection
      if (selection) {
    ...
    _refreshDataMap: function() {
      // let options = this.tableOptions
      const printTable = this.getPrintTable()
      const data = printTable.getData(this.range.s[0], this.range.s[1], printTable.countRows(), printTable.countCols())
      for (let i = 0; i < data.length; i++) {
        const row = data[i]
        for (let j = 0; j < row.length; j++) {
          const val = row[j]
          const key = this._getCellTag(i, j)
    ...
    setBackground: function(image) {
      ...
      const file = image.file || this.imageMap.background
    ...
    setCurrentCellImage: function(image) {
      const printTable = this.getPrintTable()
      const range = printTable.getSelectedRange()[0]
      if (range) {
    ...
    _getSelection() {
      const range = this.getPrintTable().getSelectedRange()
      return range ? {
    ...
    _dealFontStyle(rang, val) {
      if (rang.from.row === rang.to.row && rang.from.col === rang.to.col) {
        ...
        const selected = this.getPrintTable().getSelected()[0]
        const cell = this.getPrintTable().getCellMeta(selected[0], selected[1])
        let isExist = true
        if (cell.className) { isExist = cell.className.indexOf(val) === -1 }
        rang = this._formatRange(rang)
        for (let row = rang.from.row; row <= rang.to.row; row++) {
          for (let col = rang.from.col; col <= rang.to.col; col++) {
    ...
    _applyStyleClassName(row, col, val, d, e) {
      const cellMeta = this.getPrintTable().getCellMeta(row, col)
    ...
    _getCellBorders(range) {
      ...
      for (let row = range.from.row; row <= range.to.row; row++) {
        for (let col = range.from.col; col <= range.to.col; col++) {
          const tag = this._getCellTag(row, col)
    ...
    _dealBorderWeight(range, value) {
      ...
      for (let row = range.from.row; row <= range.to.row; row++) {
        for (let col = range.from.col; col <= range.to.col; col++) {
          let className = printTable.getCellMeta(row, col).className
          if (className) {
    ...
    setCellBorder(data, range) {
      ...
      if (range) {
        range = this._formatRange(range)
        const state = this._getCellBorders(range)
    ...
    _setOuterBorder: function(range) {
      for (let row = range.from.row; row <= range.to.row; row++) {
        for (let col = range.from.col; col <= range.to.col; col++) {
          ...
          let className = this.getPrintTable().getCellMeta(row, col).className
    ...
    _createBordersByStyle: function(row, col, style) {
      if (style) {
        ...
        if (borderStyle && borderStyle !== 'none') {
          ...
          for (let i = 0; i < this.BORDER_POSITION.length; i++) {
            const position = this.BORDER_POSITION[i]
            const d = ['border', position, 'width'].join('-')
    ...
    _isCellMerge(r1, c1, r2, c2) {
      ...
      const isCellMerge = mergeCells.some(cell => {
        return cell && cell.row === r1 && cell.col === c1 && r1 + cell.rowspan - 1 === r2 && c1 + cell.colspan - 1 === c2
      })
      return isCellMerge
    },
    /**
    ...
    merge(rang, values) {
      ...
      values.forEach((value) => {
        const tag = this._getCellTag(value.row, value.col)
    ...
    unMerge(rang) {
      ...
      const mergeCells = printTable.getPlugin('MergeCells')
      const cells = []
      mergeCells.mergedCellInfoCollection.forEach((item) => {
        if (item.row < rang.from.row || item.col < rang.from.col || item.row > rang.to.row || item.col > rang.to.col) {
          cells.push(item)
        }
      })
      cells.forEach((cell) => {
        mergeCells.mergedCellInfoCollection.removeInfo(cell.row, cell.col)
      })
      return cells
    },
    setPage(page) {
      ...
      const options = this.tableOptions || {}
      const $el = document.querySelector(`.ht_master .wtHider`)
      const rows = printTable.countRows()
      let height = 0
      let row = 0
      const cols = printTable.countCols()
      let width = 0
      let col = 0

      this.rows = []
      for (let r = 0; r < rows; r++) {
        const rowHeight = printTable.getRowHeight(r) || options.rowHeight
        height += rowHeight
      ...
      for (let c = 0; c < cols; c++) {
        const colWidth = printTable.getColWidth(c) || options.colWidth
        width += colWidth
