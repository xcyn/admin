...
class PrintUtils {
  constructor(options, images, dpi) {
    ...
    for (const key in options) {
      ...
      this.styleNameMap[JSON.stringify(style)] = key
    }
  }
}
...
Object.assign(PrintUtils.prototype, {
  ...
  getPageSize: function(option) {
    ...
    if (Array.isArray(option.size)) {
      ...
      pageSize = {
        width: Math.ceil(this.PAGE_SIZE[option.size].width * this.dpi.design / 25.4),
        height: Math.ceil(this.PAGE_SIZE[option.size].height * this.dpi.design / 25.4)
      }
    }
  ...
  getStyleNameByCell: function(cell) {
    const styleName = this.className2Style(cell.className)
    if (Utils.isEmpty(styleName)) {
      return this.DEFAULT_STYLE_NAME
    }
    const styleString = JSON.stringify(styleName)
    if (!this.styleNameMap[styleString]) {
      const styleKey = this._getStyleKey()
  ...
  className2Style: function(style) {
    ...
    const cellStyle = this._defaultCellStyle()
    const boderStyle = []
    ...
    for (let i = 0; i < styles.length; i++) {
      ...
      if (/font-size-[0-9]{1,2}/.test(val)) { // 字体
        let fontSize = val.replace('font-size-', '')
      ...
      const styleVal = this.STYLE_MAP[val]
  ...
  formatData: function(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const image = _this.images[value.image] || value.image
      const id = image.id
      if (id) {
        let imageKey = ''
