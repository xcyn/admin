...
export default {
  ...
  methods: {
    ...
    _setSelect: function(type, action, value) {
      ...
      const f = ['merge', 'unmerge', 'undo', 'redo', 'fontSize', 'fontFamily', 'borderLine', 'borderWeight']
    ...
    _formatPageValue: function(page) {
      const size = page.pageSize === 'auto' ? [this._mm2PT(page.pageWidth, 6), this._mm2PT(page.pageHeight, 6)] : page.pageSize
      return {
        size: size,
        layout: page.layout,
        margin: [
      ...
      for (const key in content) {
        ...
        switch (key) {
          case 'page':// 页面
            ...
            this.MENU.paper.page.value.layout = c.layout
