...
export default {
  ...
  methods: {
    ...
    getValue() {
      ...
      const header = this._getData(this.header)

      const footer = this._getData(this.footer)
    ...
    _getData(data) {
      ...
      dataColumns.forEach((column) => {
        let style = this.printUtils.getStyleNameByCell({
    ...
    setData(data) {
      ...
      if (this.$utils.isEmpty(dataColumns)) {
        ...
        dataColumns.forEach((column) => {
          const clz = this.printUtils.style2ClassName(column.style)
          columns.push({
            content: column.content,
            className: clz
          })
