    ...
    >
      ...
      <template v-if="formType !=='combination'">
        ...
        <el-form-item v-else label="表名:" prop="code">
          <table-config-selector v-model="formDef.code" value-key="tableName" />
        </el-form-item>
      </template>
      <el-form-item v-if="formType ==='form'" label="表单构建模式:" prop="buildMode">
...
import FormbuilderDialog from '@/business/platform/form/formbuilder/dialog'
...
var validateFormKey = (rule, value, callback) => {
  if (!testKey(value)) {
    ...
    return {
      ...
      rules: {
        ...
        key: [{ required: true, message: this.$t('validate.required') },
          { validator: validateFormKey, formType: this.formType, trigger: 'change' },
          { min: 1, max: 64, message: '长度不能超过64个字符', trigger: 'blur' }],
  ...
  methods: {
    ...
    getFormData() {
      if (this.$utils.isEmpty(this.formId)) {
        ...
        this.formDef.formType = this.formType || 'form'
        this.formValidate()
        return
      }
