...
export default {
  ...
  data() {
    return {
      ...
      targetTenantIds: [], // 设置资源分配需要使用
      dialogAssignResourceVisible: false, // 设置资源分配弹窗
  ...
  methods: {
     ...
     */
    handleResource(ids) {
      if (this.$store.getters.isTenantAdmin === true && ids.toString().match(this.$store.getters.tenants[1].id)) {
        ActionUtils.error('已勾选的租户中包含当前登录的租户,不能给自己分配资源!')
        return
      }
      this.targetTenantIds = ids.split(',')
