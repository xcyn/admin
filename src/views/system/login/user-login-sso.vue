<template>
  <el-form ref="loginForm" id="loginFormSSO":rules="loginRules" :model="loginForm" class="login-form" label-width="0"
    @submit.native.prevent>
    <el-form-item prop="username">
      <el-input v-model="loginForm.username" :placeholder="$t('system.login.username')" tabindex="1" clearable
        auto-complete="on" prefix-icon="ibps-icon-user" name="username" @keyup.enter.native="handleLogin" />
    </el-form-item>

    <el-tooltip v-model="capsTooltip" :content="$t('system.login.capsTooltip')" placement="right" manual>
      <el-form-item prop="password">
        <el-input v-model="loginForm.password" :type="passwordType" :placeholder="$t('system.login.password')"
          tabindex="2" clearable auto-complete="on" prefix-icon="ibps-icon-lock" name="password"
          @keyup.native="checkCapslock" @blur="capsTooltip = false" @keyup.enter.native="handleLogin">
          <i slot="suffix" :class="passwordType==='password'?'ibps-icon-eye-slash':'ibps-icon-eye'"
            class=" el-input__icon" @click="showPassword" />
        </el-input>
      </el-form-item>
    </el-tooltip>
    <el-form-item v-if="enabledValidCode" prop="captcha">
      <el-row :span="24">
        <el-col :span="15">
          <el-input v-model="loginForm.captcha" clearable :maxlength="code.len" :placeholder="$t('system.login.code')"
            tabindex="3" auto-complete="on" prefix-icon="ibps-icon-qrcode" @keyup.enter.native="handleLogin" />
        </el-col>
        <el-col :span="9">
    ...
    <el-form-item prop="remember">
      <el-row :span="24">
        ...
        <el-col v-if="regOpen || tenantOpen" :span="10">
          <el-button v-if="!tenantOpen" type="text" class="forget" @click="forget">{{ $t('system.login.forget') }}
          </el-button>
          <el-button v-if="tenantOpen" type="text" class="forget" @click="tenantForget">{{ $t('system.login.forget') }}
          </el-button>
          <el-divider v-if="regOpen" direction="vertical" />
          <el-button v-if="regOpen" type="text" class="register" @click="register">{{ $t('system.login.register') }}
          </el-button>
          <el-divider v-if="tenantOpen" direction="vertical" />
          <el-button v-if="tenantOpen" type="text" class="register" @click="tenantRegister">
            {{ $t('system.login.tenantRegister') }}</el-button>
        </el-col>
        <el-col v-if="!regOpen && !tenantOpen" :span="10">
...
<script>
  ...
  let loginForm = process.env.NODE_ENV === 'development' ?
    {
      username: 'admin',
      password: '******',
      captcha: '',
  ...
  const validateCode = (rule, value, callback) => {
    ...
    //   callback(new Error(I18n.t('system.login.codeCorrect')))
    // } else {
    //   callback()
    // }
  }
  ...
  export default {
    ...
    mounted() {
     ...
     // if (process.env.VUE_APP_CAS_OPEN === 'on') {
        ...
        top.module = that.getUrlParms("module") || ''
            ...
            this.login({
              ...
              ipbsHttp.getLocalUser(this.loginForm.username).then(res => {
                window.localStorage.setItem(
                  'userinfo',
                  JSON.stringify(res.data.dataResult[0])
                )
              })
                ...
                this.getSubCompany(res.data.dataResult[0].groupID, function(ret) {
                  if (res.state == 200) {
                    if (ret.data && ret.data.dataResult && ret.data.dataResult.length > 0) {
                      window.localStorage.setItem(
                        'companyInfo',
                        JSON.stringify(ret.data.dataResult[0])
    			               )
                    }
                  }
                })
