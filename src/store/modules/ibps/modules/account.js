...
export default {
  ...
  actions: {
    ...
    loginSSO({
      ...
      return new Promise(async (resolve, reject) => {
        // 开始请求登录接口
        loginSSO(form).then(async response => {
          const data = response.data
    ...
    logout({
      ...
      if (confirm) {
        ...
        MessageBox.confirm(vm.$t('system.logout.message'), vm.$t('system.logout.title'), {
            confirmButtonText: vm.$t('system.logout.confirmButtonText'),
            cancelButtonText: vm.$t('system.logout.cancelButtonText'),
            type: 'warning'
          }).then(() => {
    ...
    fedLogout({
      ...
      return new Promise(async (resolve, reject) => {
        ...
        removeTenantId()
        // 删除用户信息
        dispatch('ibps/user/setAccount', null, {
          root: true
        })
        dispatch('ibps/user/set', null, {
          root: true
        })
        // // 删除子系统
        // dispatch('ibps/system/set', null, { root: true })
        // 删除菜单
        dispatch('ibps/menu/menusSet', null, {
          root: true
        })

        // 设置注册用户账号
        // await dispatch('ibps/user/setRegOpen', null, { root: true })
        // // 获取是否开启租户
        // await dispatch('ibps/user/loadTenantOpen', null, { root: true })
        //单点登录退出处理
        setTimeout(function() {
          if (undefined != process.env.VUE_APP_CAS_SERVER_LOGIN_OUT && '' != process.env
            .VUE_APP_CAS_SERVER_LOGIN_OUT && process.env.VUE_APP_CAS_OPEN === 'on') {
            window.location.href = process.env.VUE_APP_CAS_SERVER_LOGIN_OUT + '?service=' + process.env
              .VUE_APP_BASE_API + '/' + process.env.VUE_APP_CAS_CLIENT_HOST_URL
          }
        }, 1000)
        resolve(vm)
      })
    ...
    load({
      ...
      return new Promise(async resolve => {
        ...
        // DB -> store 加载配置
        await dispatch('ibps/user/init', null, {
          root: true
        })
        // DB -> store 加载主题
        ...
        await dispatch('ibps/menu/asideLoad', null, {
          root: true
        })
