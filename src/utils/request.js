...
const $baseUrl = env.VUE_APP_PUBLIC_PATH || '/'
/**
 * 创建axios实例
 */
...
service.interceptors.request.use(
  async config => {
    ...
    config.baseURL = 'http://112.4.136.182:8086/ibps'
    if (config.gateway) {
...
service.interceptors.response.use(
  response => {
    ...
    const state = dataAxios?.state ?? dataAxios?.code ?? ''
    ...
    if (state === requestState.TOKEN_EXPIRED) {
      ...
      router.push({
        name: 'reset',
        params: {
          login_state: dataAxios.data,
          userId: dataAxios.variables.userId,
          tenantId: dataAxios.variables.tenantid,
        },
      })
    } else {
      // 错误处理
