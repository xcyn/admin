<template>
  <table id="flowSelectId" class="table table-bordered table-striped">
    <thead>
      <tr>
        <td width="50%">路径</td>
        <td width="50%">指定候选人</td>
      </tr>
    </thead>
    <tbody>
      <tr :id="nodeId" v-for="({nodeId,name}) in outgoingNodes" :key="nodeId">
        <td>
          <span>{{ name }}</span>
        </td>
        <td>
          <div style="margin: 15px 0;" />
          <el-checkbox v-model="checkAll[nodeId]" :indeterminate="isIndeterminate[nodeId]" size="mini" @change="(val)=>handleCheckAllChange(val,nodeId)">全选</el-checkbox>
          ...
          >选择用户</el-button> -->
          <div style="margin: 15px 0;" />
          <el-checkbox-group v-model="selectNodeUser1[nodeId]">
            <el-checkbox v-for="nodeUser in nodesUsersMap[nodeId]" :key="nodeUser.id" :label="nodeUser.id+''" @change="handleCheckedUsersChange(nodeUser.id,nodeId)">{{ nodeUser.name }}
            </el-checkbox>
          </el-checkbox-group>
        </td>
      </tr>
