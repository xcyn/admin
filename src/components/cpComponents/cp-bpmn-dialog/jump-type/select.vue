<template>
  <div>
    <el-form-item :label="jumpType==='select'?'选择路径:':'目标节点'">
      <el-select v-model="curNodeId" :disabled="(selectNodeId !=undefined && selectNodeId.length) > 0?true:false">
        <el-option v-for="({nodeId,name}) in outgoingNodes" :key="nodeId" :value="nodeId" :label="name" />
      </el-select>
    </el-form-item>
    <el-form-item v-if="nodeTypeMap[curNodeId] !=='end'" label="选择候选人:">
      <div>
        <el-checkbox v-model="checkAll[curNodeId]" :indeterminate="isIndeterminate[curNodeId]" size="mini" @change="(val)=>handleCheckAllChange(val,curNodeId)">全选</el-checkbox>
        ...
        <el-checkbox-group v-model="selectNodeUser1[curNodeId]">
          <el-checkbox v-for="nodeUser in nodesUsersMap[curNodeId]" :key="nodeUser.id" :label="nodeUser.id+''" @change="handleCheckedUsersChange(nodeUser.id,curNodeId)">{{ nodeUser.name }}
          </el-checkbox>
        </el-checkbox-group>
      </div>
    </el-form-item>
    <!-- 人员选择器 -->
    <ibps-employee-selector-dialog :visible="selectorVisible" :value="[]" multiple @close="visible => selectorVisible = visible" @action-event="handleSelectorActionEvent" />
  </div>
</template>
<script>
/* 用户选择器 */
...
export default {
  ...
  methods: {
    initData () {
      ...
      if (this.$utils.isNotEmpty(this.selectNodeId)) {
        this.curNodeId = this.selectNodeId;
      } else {
