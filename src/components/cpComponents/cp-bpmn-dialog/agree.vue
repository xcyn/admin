<template>
  <el-dialog :visible.sync="dialogVisible" :close-on-click-modal="false" :close-on-press-escape="false" :title="actionTitle" :class="className" class="bpmn-agree-dialog" append-to-body top="5vh" width="60%" @close="closeDialog" @open="openDialog">
    <el-form ref="form" v-loading.fullscreen.lock="dialogLoading" :element-loading-text="$t('common.loading')" :model="form" :rules="rules" label-width="120px" @submit.native.prevent>
      <!--不隐藏路径，审批意见-->
      <template v-if="noCommonJumpType">
        ...
        <!--正常跳转-->
        <template v-if="form.jumpType ==='common' && hidePath ">
          <jump-type-common v-if="variables.pathOutgoingNodes && variables.pathOutgoingNodes.length > 0" ref="jumpTypeNode" :jump-type="form.jumpType" :outgoing-nodes="variables.pathOutgoingNodes " :outgoing-nodes-users-map.sync="variables.pathOutgoingNodesUsersMap" />
        </template>
        <!--选择路径跳转-->
        <template v-else-if="form.jumpType ==='select'">
          <jump-type-select ref="jumpTypeNode" :jump-type="form.jumpType" :outgoing-nodes="variables.outgoingNodes" :outgoing-nodes-users-map.sync="variables.outgoingNodesUsersMap" :select-node-id="selectNodeId" />
        </template>
        <!--自由跳转-->
        <template v-else-if="form.jumpType ==='free'">
          <jump-type-select ref="jumpTypeNode" :jump-type="form.jumpType" :outgoing-nodes="variables.allNodeDef " :outgoing-nodes-users-map.sync="variables.allNodeDefUsersMap" />
        </template>
      </template>
      ...
      if (this.noCommonJumpType) {
        ...
        if (this.tlType == '0') {
          agreeData({
            ...
            if (this.$utils.isNotEmpty(this.variables.allNodeDefUsersMap) && a.call(this.variables.allNodeDeutilsfUsersMap) === '[object String]') {
          ...
          if (!this.$utils.isEmpty(this.variables)) {
            this.dialogLoading = false
          }
        }
