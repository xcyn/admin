...
var s_tiannet_turn_base1 = "height:16px;font-size:9pt;color:white;border:0 solid #CCCCCC;cursor:hand;background-color:#899ab2;";
//翻年、月等的按钮
var s_tiannet_turn = "width:28px;" + s_tiannet_turn_base;
var s_tiannet_turn1 = "width:38px;" + s_tiannet_turn_base1;
//关闭、清空等按钮样式
...
//日期选择控件体的样式
var s_tiannet_body = "width:180;background-color:#677a8c;display:none;z-index:10003;position:absolute;" +
  "border-left:1 solid #5e6977;border-top:1 solid #5e6977;border-right:1 solid #5e6977;border-bottom:1 solid #5e6977;";
//显示日的td的样式
var s_tiannet_day = "width:25px;height:20px;background-color:#D8F0FC;font-size:10pt;";
//字体样式
var s_tiannet_font = "color:#ffe999;font-size:9pt;cursor:hand;";
//链接的样式
var s_tiannet_link = "text-decoration:none;font-size:9pt;color:#2650A6;";
//横线
var s_tiannet_line = "border-bottom:1 solid #5e6977";
//------------------ 变量定义 ---------------------------//
...
var tiannetOutObject1;      //接收日期时间的对象
...
function setDayHM1(obj,obj1,obj2){
 tiannetOutObject = obj;
 tiannetOutObject1 = obj1;
 tiannetOutObject2 = obj2;
 m_bolShowHour = true;
 m_bolShowMinute = true;
 //如果标签中有值，则将日期和小时及分钟初始化为当前值
 var strValue = tiannetTrim(tiannetOutObject.value);
...
document.write('<div align="center" id="divTiannetTurn" style="border:0;background-color:#899ab2" Author="tiannet">');
document.write('<input type="button" style="'+s_tiannet_turn1+'" value="年↑" title="Earlier Year" onClick="tiannetPrevYear();">');
document.write('<input type="button" style="'+s_tiannet_turn1+'" value="年↓" title="Next Year" onClick="tiannetNextYear();">&nbsp;');
document.write('<input type="button" style="'+s_tiannet_turn1+'" value="月↑" title="Earlier Month" onClick="tiannetPrevMonth();">');
document.write('<input type="button" style="'+s_tiannet_turn1+'" value="月↓" title="Next Month" onClick="tiannetNextMonth();">');
document.write('</div>');
//输出一条横线
document.write('<div style="'+s_tiannet_line+'"></div>');
document.write('<table border=0 cellspacing=0 cellpadding=0  style="background-color:white" onselectstart="return false">');
document.write(' <tr style="background-color:#899ab2;font-size:10pt;color:white;height:22px;" Author="tiannet">');
...
//输出天的选择
document.write('<table border=0 cellspacing=1 cellpadding=0  style="background-color:white" onselectstart="return false">');
var n = 0;
for (var i=0;i<5;i++) { 
...
function tiannetWriteHead(){
 document.all.tiannetYearHead.innerText = tiannetYear + "年▼";//+ "年▼"
 document.all.tiannetMonthHead.innerText = tiannetMonth + "月▼";//+ "月▼"
 if( m_bolShowHour )  document.all.tiannetHourHead.innerText = " "+tiannetHour + "时▼";//+ "时▼"
 if( m_bolShowMinute ) document.all.tiannetMinuteHead.innerText = tiannetMinute + "分▼";//+ "分▼"
...
function selectObject(){
 ...
 if( tiannetYear < tiannetYearSt ) {
  for( var i = tiannetYear;i < tiannetYearSt;i ++  ){
   createOption(document.all.selTianYear,i,i )+ "年";//+ "年"
  }
  tiannetYearSt = tiannetYear;
 }
 //如果年份超过允许的最大年份，则创建对应的option
...
function tiannetSetValue(bolHideControl){
 ...
 if( m_bolShowMinute ){
  ...
  value += tiannetTimeSplit + minute+":00";
 }
 tiannetOutObject.value = value;
 if(tiannetOutObject1!=null&&tiannetOutObject1!="")
 {
  tiannetOutObject1.value = value.substring(0,10);
 }
  if(tiannetOutObject2!=null&&tiannetOutObject2!="")
 {
  tiannetOutObject2.value = value.substring(11,19);
 }
