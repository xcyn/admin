 ...
 * 2016-10-19 v1.4  修复IE11下svg右键无效BUG：https://developer.mozilla.org/en-US/docs/Web/API/GlobalEventHandlers/oncontextmenu
 */
...
(function($) {
	...
	$.fn.smartMenu = function(data, options) {
		...
		//给需要右键的对象注册右键菜单事件
		$(this).each(function() {
			//tanf 修复IE11 svg右键无效BUG：https://developer.mozilla.org/en-US/docs/Web/API/GlobalEventHandlers/oncontextmenu
			this.addEventListener('contextmenu', function (e) {
				...
				if (jqueryMenu) {
					...
					var _left = 0;
					var _top = 0;
					// 二级菜单 需要预留100 的大小，目前二级菜单都在一级菜单的 右侧，没法调到左侧
					var truelen = e.clientX + document.body.scrollLeft + parseInt(jqueryMenu.width() + 100 );   
					var truehig = e.clientY + document.body.scrollTop + parseInt(jqueryMenu.height());
					if (truelen < document.body.clientWidth) {
						_left = e.clientX + document.body.scrollLeft;
					} else {
						_left = e.clientX + document.body.scrollLeft - parseInt(jqueryMenu.width()) -100 - 5;
					}
					if (truehig < document.body.clientHeight) {
						_top = e.clientY + document.body.scrollTop;
					} else {
						_top = e.clientY + document.body.scrollTop - parseInt(jqueryMenu.height()) - 1;
					}
					
					jqueryMenu.css({
						display: "block",
						left: _left,//e.clientX + params.offsetX,
						top: _top//e.clientY + st + params.offsetY
					});
